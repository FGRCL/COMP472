...
class GreedyBestFirstSearch(SearchAlgorithmInterface):
    ...
    def find(state, goals, heuristic):
        ...
        start_node = Node(state, find_element(state, 0), 0, 0, 0, 0, 0, None)  #TODO to avoid a null exception we set the sorting key at 0, We need to double check it wont be problem with other search algorithms
        open = OpenList([start_node])
        closed = ClosedList()
